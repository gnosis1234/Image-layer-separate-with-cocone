...
class LoadImageFromFile:
    ...
    def __call__(self, results):
        ...
        if self.use_cache:
            ...
            if self.channel_order in ['rgba', 'bgra']:
                rgba = np.array(Image.open(filepath))
                img = self.rgba2rgb(rgba)
                if self.channel_order == 'bgra':
                    img = img[:,:,::-1]
                img = mmcv.imread(img)
            else:
                # img_bytes = self.file_client.get(filepath)
                ...
                #     channel_order=self.channel_order,
                #     backend=self.backend)  # HWC
                img = mmcv.imread(results[f'{self.key}'])
...
@PIPELINES.register_module()
class LoadImageFromImageList(LoadImageFromFile):
...
class LoadMask:
    ...
    Args:
        mask_mode (str): Mask mode in ['bbox', 'irregular', 'ff', 'set',
            ...
            * file_irregular: get mask from 'mask_path' and irregular holes
    ...
    def _init_info(self):
        if self.mask_mode in ['set', 'file_irregular']:
    ...
    def _get_random_mask_from_set(self, mask_idx=-1):
        if self.file_client is None:
    ...
    def _get_mask_from_image(self, masks):
    ...
    def __call__(self, results):
        ...
        elif self.mask_mode == 'file_irregular':
            tmp = self.mask_config['img_shape']
            self.mask_config['img_shape'] = results['gt_img_ori_shape']
            mask_from_path = self._get_random_mask_from_set(results['gt_img_idx'])
            mask_from_irregualar = get_irregular_mask(**self.mask_config)
            self.mask_config['img_shape'] = tmp
            mask = mask_from_irregualar | mask_from_path
        elif self.mask_mode == 'mi':
            mask = self._get_mask_from_image(results['output_mask'])
        else:
